  ...
  module Archives
    ...
    class Archives < Jekyll::Generator
      ...
      def append_enabled_date_type(meta, type, posts)
        ...
        whichs = []
        posts.each do |post| 
          whichs << post["data"]["which"]
        end
        whichs.uniq!
        which_posts = {}
        posts.each do |post| 
          which_posts[post["data"]["which"]] = [] unless which_posts.include?(post["data"]["which"])
          which_posts[post["data"]["which"]] << post
        end
        whichs.each do |which|
          @archives << Archive.new(@site, meta, type, which_posts[which]) if enabled?(type)
        end
        @archives
      end
